...
### `ContextForm.tsx`
Six-step wizard for video upload and product context fields. Posts to `POST /api/ingest/video` and `POST /analyze`. **Wired into `App.tsx`** — featured/preloaded products bypass it via a **client-only** path (no `POST /analyze`; bundled debate script in `DebateStream`), while freeform product entries route through `ContextForm` before calling the backend.
