"""
DGX Spark health checker — run before crew.py to verify system readiness.

Checks GPU temperature, GPU memory, CPU temperature, RAM, disk, and Ollama
model status. Prints a GO / NO-GO recommendation for running the debate.
"""
...
def check_gpu() -> dict[str, Any]:
    """Query nvidia-smi for temperature and memory."""
    result: dict[str, Any] = {
        "available": False,
        "temp_c": None,
        "mem_used_mib": None,
...
def print_gpu(gpu: dict[str, Any]) -> list[str]:
    """Print GPU status and return list of issues."""
    section("GPU STATUS")
...
def print_cpu(cpu: dict[str, Any]) -> list[str]:
    """Print CPU status and return issues."""
    section("CPU STATUS")
    issues: list[str] = []
...
def print_ram() -> list[str]:
    """Print RAM status and return issues."""
    section("RAM STATUS")
    issues: list[str] = []
...
def print_disk() -> list[str]:
    """Print disk status and return issues."""
    section("DISK STATUS")
    issues: list[str] = []
...
def check_ollama() -> dict[str, Any]:
    """Check which Ollama models are available and which are currently loaded."""
    result: dict[str, Any] = {
        "running": False,
        "available_models": [],
        "loaded_models": [],
...
def print_ollama(ollama: dict[str, Any]) -> list[str]:
    """Print Ollama status and return issues."""
    ...
    if ollama["loaded_models"]:
        ...
        if len(ollama["loaded_models"]) > 1:
            print(warn(f"{len(ollama['loaded_models'])} models loaded simultaneously — run `ollama stop <model>` to free memory"))
...
def print_verdict(all_issues: list[str]) -> None:
    ...
    if not all_issues:
        ...
        print("\n  Safe to run: python safe_crew.py")
    else:
        print(fail(f"NO-GO — {len(all_issues)} issue(s) detected:\n"))
        for i, issue in enumerate(all_issues, 1):
            print(f"    {i}. {issue}")
        print(f"\n  Recommended actions:")
        print(f"    1. Run `ollama stop <model>` to unload unused models")
        print(f"    2. Wait for GPU to cool (check with: nvidia-smi -l 2)")
        print(f"    3. Close unnecessary processes")
        print(f"    4. Use safe_crew.py instead of crew.py (single-model-at-a-time)")
        print(f"    5. If temp won't drop, add external cooling or reduce workload")
...
def main() -> None:
    print("=" * 50)
    print("  DGX SPARK HEALTH CHECK")
    print("  The War Room — Pre-Flight Diagnostic")
    print("=" * 50)
