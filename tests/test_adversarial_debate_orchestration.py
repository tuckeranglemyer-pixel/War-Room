...
def mock_crew_dependencies():
    ...
    with (
        patch("src.orchestration.adversarial_debate_engine.generate_personas", return_value=MOCK_PERSONAS) as p_personas,
        patch("src.orchestration.adversarial_debate_engine.deploy_swarm", return_value=MOCK_SWARM_RESULT) as p_swarm,
        patch("src.orchestration.adversarial_debate_engine.fetch_context_for_product", return_value=MOCK_EVIDENCE) as p_evidence,
    ):
